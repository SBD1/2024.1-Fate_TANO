...
def criar_enums(self):
    ...
    # Cria as Enumerações ----
    cur.execute('SELECT criar_enums();')

    conn.commit()
    cur.close()
    conn.close()

def tabelas_habilidades(self):
    conn = get_game_db_connection(self.game_number)
    cur = conn.cursor()

    # Cria as Tabelas de Habilidade ----
    cur.execute('SELECT tabelas_habilidades();')

    conn.commit()
    cur.close()
    conn.close()

def tabelas_classe(self):
    conn = get_game_db_connection(self.game_number)
    cur = conn.cursor()

    # Cria as Tabelas de Classe ----
    cur.execute('SELECT tabelas_classe();')

    conn.commit()
    cur.close()
    conn.close()

def tabelas_mapa(self):
    conn = get_game_db_connection(self.game_number)
    cur = conn.cursor()

    # Cria as Tabelas para o Mapa ----
    cur.execute('SELECT tabelas_mapa();')

    conn.commit()
    cur.close()
    conn.close()

def tabelas_personagem(self):
    conn = get_game_db_connection(self.game_number)
    cur = conn.cursor()

    # Cria as Tabelas de Personagem ----
    cur.execute('SELECT tabelas_personagem();')

    conn.commit()
    cur.close()
    conn.close()

def tabelas_he_iInimigo(self):
    conn = get_game_db_connection(self.game_number)
    cur = conn.cursor()

    # Cria Tabelas de Habilidade Evoluidade até Instancia Inimigo ----
    cur.execute('SELECT tabelas_he_iInimigo();')

    conn.commit()
    cur.close()
    conn.close()

def tabela_item(self):
    conn = get_game_db_connection(self.game_number)
    cur = conn.cursor()

    # Cria as Tabelas de Item ----
    cur.execute('SELECT tabela_item();')

    conn.commit()
    cur.close()
    conn.close()

def tabelas_ch_ea(self):
    conn = get_game_db_connection(self.game_number)
    cur = conn.cursor()

    # Cria as Tabelas de Item ----
    cur.execute('SELECT tabelas_ch_ea();')

    conn.commit()
    cur.close()
    conn.close()

def tabelas_mr_missao(self):
    conn = get_game_db_connection(self.game_number)
    cur = conn.cursor()

    # Cria as Tabelas de Item ----
    cur.execute('SELECT tabelas_mr_missao();')

    conn.commit()
    cur.close()
    conn.close()

def tabelas_md_ri(self):
    conn = get_game_db_connection(self.game_number)
    cur = conn.cursor()

    # Cria as Tabelas de Item ----
    cur.execute('SELECT tabelas_md_ri();')

    conn.commit()
    cur.close()
    conn.close()

def tabelas_di_di(self):
    conn = get_game_db_connection(self.game_number)
    cur = conn.cursor()

    # Cria as Tabelas de Item ----
    cur.execute('SELECT tabelas_di_di();')

    conn.commit()
    cur.close()
    conn.close()

def fuction_create_table(self):
    conn = get_game_db_connection(self.game_number)
    cur = conn.cursor()
    ...
    # -------------------- Habilidade Evoluida até Instancia Inimigo --------------------
    cur.execute('''
...
''')
    ...
    # --------------------- Consumível Habilidade até Evolução Arma -------------------
    cur.execute('''
    ...
    # --------------------- Missão Doing até Recompensa Item -----------------------
    cur.execute('''
